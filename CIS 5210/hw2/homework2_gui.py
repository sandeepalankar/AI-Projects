...
class NQueens(tk.Frame):
    def __init__(self, parent, n):
        ...
        self.__prev_btn = tk.Button(self.__buttons, text='Previous solution',
                                    command=partial(self.__display, -1))
        self.__next_btn = tk.Button(self.__buttons, text='Next solution',
                                    command=partial(self.__display, 1))
        ...
        tk.Label(self.__buttons, textvariable=self.__label
    ...
    def __display(self, delta):
        ...
        for i, pos in enumerate(self.__solutions[self.__cur_sol]):
            self.__queens.append(self.__canvas.create_image(
                (i * self.__ppb, pos * self.__ppb), image=self.__queen_image,
                anchor=tk.NW))
...
class LightsOut(tk.Frame):
    def __init__(self, parent, rows, cols):
        ...
        self.__canvas.config(width=cols * SQUARE_SIZE, height=rows * 
                             SQUARE_SIZE)
        self.__canvas.pack()
        self.__update()
...
class GUI(tk.Tk):
    def __init__(self):
        ...
        second_menu.add_command(label='Linear Disks',
                                command=self.linear_disks)
